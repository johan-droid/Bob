...
function renderList(containerId, items) {
  ...
  if (!items.length) {
    el.innerHTML =
      '<div style="padding: 32px; text-align: center; color: var(--on-surface-variant); font-size: 13px; font-weight: 500; border: 1px dashed var(--outline); border-radius: 16px;">System optimal. No pending tasks.</div>';
    return;
  }
  el.innerHTML = items
    .map((pr) => {
...
function renderRepos(repos) {
  ...
  if (!repos.length) {
    el.innerHTML =
      '<div class="text-label" style="grid-column: 1/-1; padding: 64px; text-align: center;">No integration links established.</div>';
    return;
  }
  ...
  el.innerHTML = repos
    .map((r, idx) => {
      const isBig =
        r.issue_count > 0 || r.permissions_level === "admin" || idx === 0;
