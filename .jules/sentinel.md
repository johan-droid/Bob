...
## 2026-06-11 - [Path Traversal in React Fallback Route]
**Vulnerability:** A path traversal vulnerability existed in the `react_fallback` route because it used `.startswith(frontend_root)` for path validation. If `frontend_root` lacked a trailing slash (e.g., `/app/frontend`), an attacker could request `../frontend_secrets.txt`, which normalizes to `/app/frontend_secrets.txt` and successfully evaluates `startswith('/app/frontend')` as `True`.
**Learning:** String prefix comparison is insufficient for secure path validation because it ignores directory boundaries. Additionally, manually joining paths and validating them is an anti-pattern when secure framework abstractions exist.
**Prevention:** Always use Flask's native `send_from_directory(directory, path)` which handles path normalization and traversal prevention securely out of the box, instead of attempting manual path validation logic.
