...
def test_get_open_prs_returns_list(scanner):
    with patch('requests.get', return_value=make_resp([{'number': 1, 'title': 'Fix bug', 'html_url': 'http://x'}])):
...
def test_scan_workflow_failures_parses_runs(scanner):
    ...
    with patch('requests.get', return_value=make_resp(data)):
...
def test_rate_limit_sleep_triggered(scanner):
    ...
    with patch('requests.get', return_value=resp), patch('time.sleep') as mock_sleep:
